...
export const useArchitectureQuery = (architectureId: string) =>
  useQuery({
    ...
    queryFn: async () => {
      ...
      return data.architectures.find((architecture) => architecture.id === architectureId) ?? null
    },
  })
...
export const useTasksQuery = (architectureId: string) =>
  useQuery({
    ...
    queryFn: async () => {
      const data = await loadPanelData()
      return data.tasks.filter((task) => task.architectureId === architectureId)
    },
  })
...
export const useTrayFeedQuery = () =>
  useQuery({
    queryKey: ['tray-feed'],
    queryFn: async () => (await loadPanelData()).trayFeed,
  })